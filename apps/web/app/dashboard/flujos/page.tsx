...
import { useFlows, useToggleFlow, useCreateFlow, useDeleteFlow, Flow, FlowExecution, FlowStep } from "@/hooks/use-flows";
...
function stepDescription(step: { type: string; config?: Record<string, any> }) {
  ...
  if (step.config?.templateName) return `${label} · plantilla: ${step.config.templateName}`;
  if (step.config?.subject) return `${label} · ${step.config.subject}`;
...
function FlowCard({ flow }: { flow: Flow }) {
  const { mutate: toggle, isPending: toggling } = useToggleFlow();
  const { mutate: deleteFlow, isPending: deleting } = useDeleteFlow();
  ...
  const handleDelete = () => {
    deleteFlow(flow.id, {
      onSuccess: () => toast.success(`Flujo "${flow.name}" eliminado`),
    ...
    <div className="bg-slate-100 border border-slate-200 rounded-2xl p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-[11px] text-slate-400 font-medium">{todayExecs.length} hoy</span>
          <button
...
function ExecRow({ exec, flowName }: { exec: FlowExecution & { flowName: string }; flowName: string }) {
  const initial = exec.contactId.slice(0, 2).toUpperCase();
  const dotColor = EXEC_STATUS_DOT[exec.status] ?? "bg-slate-300";
  const statusLabel: Record<string, string> = {
    running: "En proceso", completed: "Completado", failed: "Error", cancelled: "Cancelado",
  };
  ...
  return (
    <div className="flex items-center gap-4 p-3 rounded-xl hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs font-semibold text-slate-700 truncate font-mono">{exec.contactId}</p>
        <p className="text-[11px] text-slate-400 truncate">
          Flujo: {flowName} · {statusLabel[exec.status] ?? exec.status}
        </p>
      </div>
...
function StepConfigFields({
  ...
  if (step.type === "send_whatsapp") return (
    <div className="space-y-1.5 mt-1.5">
      <Input
        ...
        onChange={(e) => onChange({ ...cfg, templateName: e.target.value })}
        className="text-xs h-8"
      />
      <p className="text-[10px] text-slate-400">
        La plantilla debe estar aprobada por Meta. Usá el nombre exacto (ej. <code>saludo_inicial</code>)
      </p>
    </div>
  );
  ...
  if (step.type === "send_email") return (
    <div className="space-y-1.5 mt-1.5">
      ...
      />
      <Textarea
        placeholder="Cuerpo del email (puede incluir saltos de línea)"
        value={cfg.content ?? ""}
  ...
  if (step.type === "update_status") return (
    <select
      ...
      className="w-full h-8 mt-1.5 rounded-md border border-slate-200 px-2 text-xs bg-white"
    >
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-lg bg-white max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 py-2">
          ...
          {/* Trigger */}
          <div>
            <label className="text-xs font-semibold text-slate-500 mb-1 block">Disparador</label>
            <select
              value={trigger}
            ...
            <p className="text-[10px] text-slate-400 mt-1">
              El flujo se activará automáticamente cuando ocurra este evento en un contacto.
            </p>
          </div>
      ...
      <div className="flex items-center justify-between max-w-7xl mx-auto">
        <div>
          ...
          <p className="text-[11px] text-slate-400 font-medium">
            Se ejecutan automáticamente según el trigger configurado
          </p>
        </div>
        <NewFlowModal />
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-4">
        ...
        {recentExecs.length > 0 && (
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm mt-8">
            <h3 className="text-sm font-bold text-slate-800 mb-6 tracking-tight">
